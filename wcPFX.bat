...
:wildcard
set "domain=%domain:~2%"
for /f tokens^=2^ delims^=^" %%i in ('appcmd list site^|find/i "%domain%"') do (
    echo Updating certificate for site: %%i
    appcmd set config -section:system.applicationHost/sites /[name='%%i'].bindings.[protocol='https'].certificateHash:"%t%" /commit:apphost
    appcmd set config -section:system.applicationHost/sites /[name='%%i'].bindings.[protocol='https'].certificateStoreName:"My" /commit:apphost
)
netsh http delete sslcert ipport=0.0.0.0:443 >nul 2>&1
netsh http add sslcert ipport=0.0.0.0:443 certhash=%t% appid={4dc3e181-e14b-4a21-b022-59fc669b0914} >nul 2>&1
echo All sites matching *.%domain% updated successfully.
